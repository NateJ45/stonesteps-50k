...
- `npm run map-poster` to re-bake the home page's still of the course map into `public/course-poster*.{avif,webp}` plus `scripts/data/course-poster.json`. It drives the BUILT site with Playwright (`npm run build` first), opens `/course?poster=1`, which strips the map to its photograph, and captures the picture AND the route projected into that same picture's pixels in one page load. The two halves have to come from the same load: computing the overlay separately is how a line ends up a few pixels off the photograph it sits on. Re-run it after any change to the map's camera, terrain exaggeration or imagery source, and commit the output. The camera can be auditioned with `?poster=1&z=&p=&b=&dx=&dy=` before changing the committed default in `CourseMapLibre.tsx`.
...
- `npm run parity list | capture | compare [page]` runs `scripts/page-parity.mjs`, the rendered-HTML parity harness. It never builds: build first, then capture or compare. Routes are auto-discovered from the build output and baselines live in `scripts/.parity/` (committed). Use it on any change that is meant to be render-neutral. See PORTS.md card 3.
  - **Build with `PUBLIC_GA_ID=` blank, or parity fails on every page for a reason that has nothing to do with your change.** `.env` carries the real Measurement ID, Astro loads `.env` automatically, and the committed baselines have no analytics tag in them. So a plain `npm run build` injects the GA script into all 32 pages and `compare` comes back 1/32 with a thousand-line diff. That is the harness working correctly on a build that is not comparable, not a regression. The honest command for a parity run here is `PUBLIC_GA_ID= npm run build && npm run parity compare`. Cost the 2026-09-18 session two wrong diagnoses before the `.env` line was found.
