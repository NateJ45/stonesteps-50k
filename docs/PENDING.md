...
### 1i. The home page's course-map poster needs adding to the Home Page document

2026-09-16. The band is built, tested and deployed, but the home page is
`pageBuilder`-driven from Sanity, so a section that exists in code appears on the
live page only when an editor puts it there. Nothing is broken until then; the
home page simply carries the elevation band and no map.

To add it: Studio, Home Page, page builder, insert **Course map poster**. It has
optional eyebrow, headline and intro and falls back to its own copy if they are
left empty. Recommended position is directly after the elevation band, so the
page answers "how hilly" and then "where does it go" in that order.

There is deliberately no image field on the block: the picture is generated from
the map by `npm run map-poster` and committed. An editor who could swap it could
put a photograph of anywhere behind a line drawn from this course's coordinates,
